...
class HTTPRequest:
    ...
    def read_headers(self):
        ...
        head = data[0].split(" ")
        self.method = head[0]
        self.request_target = head[1]
        self.http_version = head[2]
        for i in range(1, len(data) - 2):
            header = data[i].split(": ")
            self.headers.append(HTTPHeader(header[0], header[1]))
        self.body_length = len(data[len(data) - 1])
        self.buffer = bytearray(data[len(data) - 1].encode())
    ...
    def read_message_body(self) -> bytes:
        ...
        for h in self.headers:
            if h.name == 'Content-Length':
                length = int(h.value) - 1024
        content = self.buffer.decode()
        while length > 0:
            content += self.socket.recv(1024).decode()
            length -= 1024
        self.buffer = bytearray(content.encode())
        return self.buffer
...
class HTTPResponse:
    ...
    def write_all(self):
        ...
        status_line = ""
        status_line += self.http_version
        status_line += " "
        status_line += str(self.status_code)
        status_line += " "
        status_line += self.reason
        status_line += "\r\n"
        message = ""
        for i in self.headers:
            ...
            message += '\r\n'
        message += "\r\n"
        self.socket.sendall(status_line.encode() + message.encode() + self.body)
