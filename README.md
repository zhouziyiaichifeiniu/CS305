...
数据平面：每台路由器的功能，决定到达路由器输入链路之一的数据报如何转发到路由器的输出链路之一。

软件定义网络Software-Defined Networking SDN
...
* 转发：数据平面
* 路由选择：控制平面        路由选择算法routing algorithm
...
最长前缀匹配规则（longest prefix matching rule）  三态内容可寻址存储器（Tenary Content Address Memory）
...
线路前部阻塞 Head-Of-the-Line

2. 输出排队

要么丢弃到达的分组（弃尾drop-tail）；要么删除一个或多个已排队的分组为新来的分组腾出空间
...
跨越两个AS的BGP连接成为外部eBGP，相同AS中的路由器会话称为内部iBGP

通过eBGP告知自己的可达性，再通过iBGP告知子网内所有路由器

#### 5.4.3 确定最好的路由

1. 热土豆路由选择

对于一个路由器，用最低开销将分组送出AS
